...
void test_ioctl_commands(void)
{
    ...
    for (size_t i = 0; i < sizeof(cmds) / sizeof(cmds[0]); ++i) {
        TEST_ASSERT_NOT_EQUAL(0, cmds[i]);
        for (size_t j = i + 1; j < sizeof(cmds) / sizeof(cmds[0]); ++j) {
    ...
    TEST_ASSERT_EQUAL_UINT(_IOC_WRITE, _IOC_DIR(GATEWAY_MONITOR_IOC_SET_PEAK_THR));
    TEST_ASSERT_EQUAL_UINT(_IOC_WRITE, _IOC_DIR(GATEWAY_MONITOR_IOC_SET_RMS_THR));
    TEST_ASSERT_EQUAL_UINT(_IOC_WRITE, _IOC_DIR(GATEWAY_MONITOR_IOC_SET_GYRO_THR));
    TEST_ASSERT_EQUAL_UINT(_IOC_READ, _IOC_DIR(GATEWAY_MONITOR_IOC_GET_PEAK_THR));
    TEST_ASSERT_EQUAL_UINT(_IOC_READ, _IOC_DIR(GATEWAY_MONITOR_IOC_GET_RMS_THR));
    TEST_ASSERT_EQUAL_UINT(_IOC_READ, _IOC_DIR(GATEWAY_MONITOR_IOC_GET_GYRO_THR));
}
