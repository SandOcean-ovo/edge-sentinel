...
bool RingBuf_write(RingBuf_t *pbuf, uint8_t data)
{
    ...
    if (next_write >= pbuf->size)
        next_write = 0; // 单纯的判断，比取模运算高效
    if (next_write == pbuf->tail)
        return false; // 判满
...
{
    if (pbuf->tail == pbuf->head || RingBuf_getreadable(pbuf) < offset + 1)
        return false; // 判空

    if (pdes != NULL)
    {
        uint32_t read = (pbuf->tail + offset) % pbuf->size;
        *pdes = pbuf->pdata[read];
    }
    return true;
}
...
{
    uint32_t readable = RingBuf_getreadable(pbuf);
    if (len > readable)
        len = readable;

    // 直接通过数学计算挪动 tail 索引，不需要 memcpy
    pbuf->tail = (pbuf->tail + len) % pbuf->size;
